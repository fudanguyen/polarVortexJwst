...
class AtmosphericModel:
    ...
    """
    def __init__(self, mesh, band_config, vortex_config, phase_values=None):
        """
        Args:
        ...
        self.phase_values = 3*np.array([1.3, 1.0, 0.7, 0.1, 0.9, 0.4, 1.2, 0.4, 1.3, 0.9, 0.5,])
    ...
    def _circle_vortice_vectorized(self, im, lat1, lat2, t, group, modu_config, #band_config
        ...
        if modu_config == 'polarDynamic':
            ...
            variableflux = np.array(vortex_amp * np.sin(2 * np.pi / period * t + phase 
                                                        + self.phase_values[:self.n_vortice]))
...
class AtmosphereVisualizer:
    ...
    """
    def __init__(self, mesh, speckey, config, imsize=(600, 600), inclination=0):
        self.mesh = mesh
...
def renderer(runName, sim_config, band_config=None, time_config=None, vortex_config=True, phase_values=None):
    print("VTK Version:", vtk.vtkVersion.GetVTKVersion())
    print("OpenGL2 Enabled:", hasattr(vtk, 'vtkOpenGLRenderWindow'))

    output_dir = os.path.join('output', runName)
    ...
    if band_config == None:
        ...
        # Extract amplitude config from user-provided band_config
        # Expect format: [amp_config_dict, band_config_array]
        amp_config = band_config[0]
        band_config_array = band_config[1]
        
        # Extract values from amp_config
        Prot = amp_config['Prot']
        Fpolar = amp_config['Fpolar']
        Fband = amp_config['Fband']
        Fambient = amp_config['Fambient']
        Fpolar_var = amp_config['Fpolar_var']
        Fband_var = amp_config['Fband_var']
        Fambient_var = amp_config['Fambient_var']
        
        # Replace band_config with just the array
        band_config = band_config_array
    ...
    elif vortex_config == False:
        vortex_config = None

    # UNPACK SIMULATION DICT

    modu_config = sim_config['modu_config']
    incli_array = sim_config['inclinations']
    resolution = sim_config['mesh_resolution']
    plot_paths = sim_config['plot_paths']
    plot_lc = sim_config['plot_lc']

    # CREATE CLASS OBJECTS
    
    timeConfig = TimeConfig(t0=t0, t1=t1, frames=frames, option=option)

    atmoConfig = AtmosphericConfig(
        band_config=band_config_array,  # This is your band configuration list
    ...
    mesh = SphericalMesh(resolution=resolution)

    if modu_config == 'polarDynamic':
        if phase_values != None:
            model = AtmosphericModel(mesh, atmoConfig, vortex_config, phase_values=phase_values)
    else:
        model = AtmosphericModel(mesh, atmoConfig, vortex_config)
    ...
    def plot_frames(h5_path, runName, inclination, t=0, handle='gray', plot_discrete=True, bins=None):
    ...
    if True:
        filepath = os.path.join(output_dir, f'{runName}.h5')
        for inc in incli_array:
            for t in range(1):
                binned = plot_frames(filepath, runName, inclination=inc, t=3*t, bins=bins)
...
if __name__ == "__main__":
    ...
    renderer('main_v2', sim_config, vortex_config=True)
